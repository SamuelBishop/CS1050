...
{
	int i;
	Car *Cars = malloc(sizeof(Car)*size);
	for(i = 0; i < size; i++)
	{
		...
		printf("Enter the amount of miles on the car: ");
		scanf("%d", &(Cars + i)->miles);
...
*/
void printInfo(int size, Car* Car)
{
	int i;
	int n = 1;
	for (i = 0; i < size; i++)
	{
		...
		printf("Car %d miles: %d\n", n, ((Car + i)->miles));
		printf("Car %d price: %.2f\n", n, ((Car + i)->price));
		n++;
	}
}
